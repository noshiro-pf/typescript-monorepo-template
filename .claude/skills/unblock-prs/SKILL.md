...
- **`type-check (ws:test:browser)` needs Playwright** — `pnpm exec playwright
install chromium` — and CI retries it once before calling it failed, so a
  single red run there may just be flake. Re-read the log before changing code.
- **Every `style-check` and `type-check` job ends with `assert-repo-is-clean`.**
  So `fmt:full`, `ws:doc`, `codemod:full` and `ws:lint:fix` fail by _changing_
