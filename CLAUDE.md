...
`true`, and do not teach the script to cross a major: `changesets/action`
renamed every input at v2, so taking that major unattended leaves
`release.yml` passing the old names and breaks the release on main.
...
compatibility matrix, which is why its test runs from `pnpm run check:root:test` rather
